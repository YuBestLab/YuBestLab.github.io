classdef HotMapCreaterApp < handle
    %
    % Usage:
    %     app.HotMapCreaterApp
    ...
    properties (Constant)
        AppPathBase = {'/HotMapCreater'};
        AppClass = 'HotMapCreaterApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            HotMapCreaterApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'HotMap_Creater')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong HotMap_Creater
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'HotMap_Creater').getString, ...
            ...
            if nargout(@HotMap_Creater) == 0
                eval('HotMap_Creater');
            else
                obj.AppHandle = eval('HotMap_Creater');
            end
